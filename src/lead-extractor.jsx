...
// ── Maps scraper via Google Places API ────────────────────────────────────
const scrapeWithPlaces = async (apiKey, query, location, onProgress) => {
  ...
  const searchRes = await fetch(
    `https://places.googleapis.com/v1/places:searchText`,
    {
      ...
      headers: {
        "Content-Type": "application/json",
        "X-Goog-Api-Key": apiKey,
        "X-Goog-FieldMask": "places.id,places.displayName,places.formattedAddress,places.internationalPhoneNumber,places.websiteUri,places.rating,places.userRatingCount,places.primaryTypeDisplayName",
      },
      body: JSON.stringify({ textQuery: searchText, maxResultCount: 20 }),
    }
  );
  const searchData = await searchRes.json();
  if (searchData.error) throw new Error(searchData.error.message);
  const places = searchData.places || [];
  if (!places.length) throw new Error("No businesses found. Try a different query.");

  onProgress(`Found ${places.length} businesses…`);
  return places.map(p => ({
    id: uid(),
...
export default function LeadExtractor() {
  ...
  const exportXLSX = (rows = displayLeads) => {
    ...
    // Column widths
    ws["!cols"] = [{ wch: 24 }, { wch: 32 }, { wch: 24 }, { wch: 22 }];
  ...
  const runMapsScrape = async () => {
    ...
    try {
      const results = await scrapeWithPlaces(placesKey.trim(), mapsQuery.trim(), mapsLocation.trim(), setMapsProgress);
      ...
      <div style={{ maxWidth: 960, margin: "0 auto", padding: "22px 18px" }}>
            ...
            {mode === "file" && (
              <label htmlFor="main-file-input" style={{ display: "block", border: `2px dashed ${file ? C.accent : C.border}`, borderRadius: 6, padding: "26px 20px", textAlign: "center", cursor: "pointer", background: file ? `${C.accent}07` : "transparent", transition: "all .2s" }}>
                ...
                <div style={{ fontSize: 12, color: file ? C.accent : C.muted }}>
                  {file ? `✓  ${file.name}  (${(file.size / 1024).toFixed(1)} KB)` : "Tap to choose — CSV, TXT, PDF"}
                </div>
                {file && (
                  <div
                    onClick={e => { e.preventDefault(); e.stopPropagation(); setFile(null); }}
                    style={{ marginTop: 8, fontSize: 10, color: C.danger, cursor: "pointer" }}
                  >
                    ✕ remove
                  </div>
                )}
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 6, padding: 18, marginBottom: 16 }}>
            {bulkInputMode === "text" ? (<>
              ...
              <label htmlFor="bulk-file-input" style={{ display: "block", border: `2px dashed ${bulkFile ? C.accent : C.border}`, borderRadius: 6, padding: "26px 20px", textAlign: "center", cursor: "pointer", background: bulkFile ? `${C.accent}07` : "transparent", transition: "all .2s" }}>
                <div style={{ fontSize: 26, marginBottom: 7 }}>📂</div>
                <div style={{ fontSize: 12, color: bulkFile ? C.accent : C.muted }}>{bulkFile ? `✓  ${bulkFile.name}  (${(bulkFile.size / 1024).toFixed(1)} KB)` : "Tap to choose — CSV or TXT"}</div>
              </label>
              <input
                id="bulk-file-input"
                ref={bulkFileRef}
              ...
              <div style={{ maxHeight: 460, overflowY: "auto" }}>
                {bulkResults.map((row, i) => (
                  <div key={i} className="bulk-row fade-row" style={{
                    ...
                    <span style={{ fontSize: 10, color: row.status === "ok" ? C.accent : row.status === "error" ? C.danger : row.status === "none" ? "#1e2840" : C.dim }}>
                      {row.status === "loading" ? "…" : row.status === "ok" ? `✓ ${row.emails.length}` : row.status === "error" ? "✕ fail" : "— none"}
                    </span>
                  </div>
                ))}
              </div>
        ...
        {tab === "gemini" && (<>
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 6, padding: 18, marginBottom: 16 }}>
            ...
            <div style={{ fontSize: 10, color: C.dim, lineHeight: 1.6 }}>
              Get a free key at <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noreferrer">Google AI Studio</a>. Free tier: 15 requests/min, 1,500/day.
            </div>
          </div>
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 6, padding: 18, marginBottom: 16 }}>
            {geminiInputMode === "text" ? (<>
              ...
              <label htmlFor="gemini-file-input" style={{ display: "block", border: `2px dashed ${geminiFile ? C.purple : C.border}`, borderRadius: 6, padding: "26px 20px", textAlign: "center", cursor: "pointer", background: geminiFile ? `${C.purple}07` : "transparent", transition: "all .2s" }}>
                <div style={{ fontSize: 26, marginBottom: 7 }}>📂</div>
                <div style={{ fontSize: 12, color: geminiFile ? C.purple : C.muted }}>{geminiFile ? `✓  ${geminiFile.name}  (${(geminiFile.size / 1024).toFixed(1)} KB)` : "Tap to choose — CSV or TXT"}</div>
              </label>
              <input
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 14, alignItems: "center" }}>
              {!geminiRunning ? (
                <button className="run-btn" onClick={runGeminiScrape}
                  ...
                  style={{
                    padding: "9px 26px",
                    background: geminiKey.trim() && (geminiInputMode === "text" ? parseGeminiUrls(geminiUrls).length : geminiFile) ? C.purple : C.surface2,
                    color: geminiKey.trim() && (geminiInputMode === "text" ? parseGeminiUrls(geminiUrls).length : geminiFile) ? "#000" : C.muted,
                    border: "none", borderRadius: 4, cursor: "pointer", fontFamily: "inherit",
                    fontWeight: 500, fontSize: 11, letterSpacing: ".1em", textTransform: "uppercase",
                    transition: "all .15s", display: "flex", alignItems: "center", gap: 8,
                  }}>✦  Start Gemini Scrape</button>
              ) : (
                <button onClick={() => { geminiStopRef.current = true; }} style={{ padding: "9px 22px", background: C.danger, color: "#fff", border: "none", borderRadius: 4, cursor: "pointer", fontFamily: "inherit", fontWeight: 500, fontSize: 11, letterSpacing: ".1em", textTransform: "uppercase" }}>
                  ◼  Stop
                </button>
              )}
              {geminiRunning && (
        ...
        {tab === "maps" && (<>
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 6, padding: 18, marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 6 }}>
              <input
                ...
                style={{ flex: 1, background: "#070b0f", border: `1px solid ${C.border}`, borderRadius: 4, color: C.text, fontFamily: "inherit", fontSize: 12, padding: "9px 12px" }}
              />
              <button onClick={() => setPlacesKeySaved(true)} disabled={!placesKey.trim()} style={{
          ...
          <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 6, padding: 18, marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <button className="run-btn" onClick={runMapsScrape} disabled={mapsLoading || !mapsQuery.trim() || !placesKey.trim()} style={{
                padding: "9px 26px", background: !mapsLoading && mapsQuery && placesKey ? C.purple : C.surface2,
                color: !mapsLoading && mapsQuery && placesKey ? "#000" : C.muted,
                border: "none", borderRadius: 4, cursor: "pointer", fontFamily: "inherit",
                fontWeight: 500, fontSize: 11, letterSpacing: ".1em", textTransform: "uppercase",
                transition: "all .15s", display: "flex", alignItems: "center", gap: 8,
